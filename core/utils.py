...
class CacheManager:
    ...
    def get(cls, cache_type, *args):
        ...
        try:
            ...
            if datetime.now() > datetime.fromisoformat(data.get('expire_time', '2000-01-01')):
                cache_file.unlink()
                return None
...
class HttpClient:
    @staticmethod
    async def request(method, url, headers=None, params=None, json_data=None, use_proxy: bool = False):
    ...
    def check_for_updates():
        ...
        try:
            with console.status(f"[bold cyan]{Language.get('update_check')}", spinner="dots"):
                ...
                if UpdateManager.is_newer_version(latest_tag, APP_VERSION): found_update = True
        except: pass
