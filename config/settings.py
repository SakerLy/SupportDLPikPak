...
class Language:
    ...
    def get(cls, key):
        ...
        return cls.STRINGS.get(lang, cls.STRINGS["en"]).get(
...
class Config:
    ...
    def test_proxy(cls) -> tuple:
        ...
        try:
            ...
            if "407" in err or "Proxy Authentication" in err:
                return False, "Proxy yêu cầu xác thực"
            if "No module" in err and "socks" in err.lower():
                return False, "Thiếu thư viện requests[socks]"
            if "SOCKS" in err or "socks" in err.lower():
                return False, f"SOCKS lỗi loại proxy ({cls.PROXY_TYPE})"
        ...
        try:
            ...
            if "SSLError" in err or "CONNECT" in err or "tunnel" in err.lower():
                return False, "Không hỗ trợ HTTPS tunnel"
            if "407" in err or "Proxy Authentication" in err:
                return False, "Yêu cầu xác thực"
    ...
    def setup_dirs(cls):
        try:
            cls.get_download_dir().mkdir(parents=True, exist_ok=True)
        except:
            pass
