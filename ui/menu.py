...
from core.utils import UpdateManager, CacheManager, HttpClient
...
def _run(coro):
    # asyncio.run tạo event loop mới mỗi lần, nên phải đóng session HTTP dùng chung
    # trước khi loop chết để không rò rỉ socket
    async def _wrapped():
        try:
            return await coro
        finally:
...
class Menu:
    ...
    async def _startup(self) -> int:
        ...
        if n:
            ...
            await CloudGarbageCollector().run()
        return n
    ...
    def print_header(self):
        ...
        console.print(
            Align.center(
                f"[bold white]VERSION {APP_VERSION}[/] | [dim]{Language.get('menu_dev')}[/]\n"
            )
        )
    ...
    def main_menu(self):
        ...
        Config.load_config()
        n = _run(self._startup())
        if n > 1:
            console.print(
                f"  [bold green]✓ Account pool: {n} accounts active (~{n * 11} MB/s max)[/]"
            )
            time.sleep(1)
        while True:
            Config.load_config()
            ...
            console.print()
            c = Prompt.ask(
                f"[bold green]👉 {Language.get('prompt_choice')}[/]",
    ...
    def login_with_password(self):
        ...
        try:
            with console.status(
                f"[bold cyan]  {Language.get('login_wait')}", spinner="dots"
            ):
                result = _run(api.login())
        except:
            result = None
        if not result:
    ...
    def settings_menu(self):
        while True:
            ...
            table.add_row(
                "[2]", "Số luồng/file", f"[cyan]{Config.CONCURRENT_THREADS} conn[/]"
            )
            table.add_row("[3]", "Cài đặt nâng cao", "")
            table.add_row("[4]", "Ngôn ngữ", f"[dim]{Config.LANGUAGE.upper()}[/]")
            table.add_row(
                "[5]", "Premium Transfer Mode", self._badge(Config.FORCE_PREMIUM_MODE)
            )
            ...
            if c == "1":
                ...
                self.premium_mode_setup()
            elif c == "6":
                self._toggle_cache_quick()
            elif c == "0":
                break
    ...
    def _toggle_cache_quick(self):
        ...
        Config.save_config()
        console.print(
            f"  [bold green]✓ Đã lưu![/]  Cache: {self._badge(Config.USE_CACHE)}"
        )
        time.sleep(1)
    ...
    def proxy_setup(self):
        while True:
            ...
            if Config.USE_PROXY and Config.PROXY_HOST:
                ...
                grid.add_row(
                    "Địa chỉ:  ",
                    f"[white]{Config.PROXY_TYPE}://{auth_part}{Config.PROXY_HOST}:{Config.PROXY_PORT}[/]",
                )
            console.print(grid)
            console.print()
            ...
            if c == "0":
                ...
                Config.save_config()
                console.print(
                    f"  [bold green]✓ Đã lưu![/]  Proxy: {self._badge(Config.USE_PROXY)}"
                )
                time.sleep(1)
            elif c == "2":
                console.print(
                ...
                console.print(
                    "  [dim]  http   — phổ biến nhất, nhưng nhiều proxy free KHÔNG hỗ trợ HTTPS[/]"
                )
                console.print(
                    "  [dim]  socks5 — hỗ trợ cả HTTP lẫn HTTPS, ổn định hơn[/]"
                )
                ...
                if Config.PROXY_TYPE == "http":
                    console.print(
                        "  [bold yellow]⚠ HTTP proxy có thể không tải được từ PikPak (HTTPS). Nếu lỗi hãy thử socks5.[/]"
                    )
                ...
                if ok:
                    ...
                    console.print(
                        "  [dim]1. Đổi loại proxy sang [bold]socks5[/bold] (hỗ trợ HTTPS tốt nhất)[/]"
                    )
                    console.print(
                        "  [dim]2. Kiểm tra proxy còn sống: proxynova.com/proxy-checker[/]"
                    )
                    console.print(
                        "  [dim]3. Proxy HTTP free thường không tunnel được HTTPS[/]"
                    )
                Prompt.ask("\n  Enter để tiếp tục...")
    ...
    def threads_setup(self):
        ...
        grid.add_column()
        grid.add_row(
            "Luồng/file hiện tại:", f"[cyan]{Config.CONCURRENT_THREADS}[/] connections"
        )
        grid.add_row(
            "Tốc độ ước tính:    ",
            f"[cyan]~{Config.CONCURRENT_THREADS * 11} MB/s[/] tối đa",
        )
    ...
    def premium_mode_setup(self):
        ...
        grid.add_row(
            "Khi BẬT:  ", "[dim]Restore TẤT CẢ file lên cloud trước khi tải[/]"
        )
        grid.add_row("Khi TẮT:  ", "[dim]Chỉ restore video/zip/rar/iso (mặc định)[/]")
        console.print(grid)
        Config.FORCE_PREMIUM_MODE = self._ask_toggle(
            "Premium Mode", Config.FORCE_PREMIUM_MODE
        )
        Config.save_config()
        console.print(
    ...
    def advanced_setup(self):
        ...
        try:
            ...
            pass
        Config.DOWNLOAD_PATH_STR = Prompt.ask(
            "  Thư mục lưu", default=Config.DOWNLOAD_PATH_STR
        )
        try:
    ...
    def cache_menu(self):
        ...
        if Confirm.ask(f"\n  [bold red]{Language.get('cache_clear')}[/]"):
            ...
            console.print(f"  [bold green]{Language.get('cache_cleared')}[/]")
            time.sleep(1)
    ...
    def view_config(self):
        ...
        grid.add_row(
            "Proxy:",
            f"{Config.PROXY_HOST}:{Config.PROXY_PORT}" if Config.USE_PROXY else "Off",
        )
        grid.add_row("Prem Mode:", "ON" if Config.FORCE_PREMIUM_MODE else "Off")
        grid.add_row("Threads/File:", str(Config.CONCURRENT_THREADS))
        console.print(
            Panel(
                grid, title=Language.get("menu_5"), border_style="blue", box=box.ROUNDED
            )
        )
        Prompt.ask("\n  [dim]Enter...[/]")
    ...
    def _add_extra_account(self):
        ...
        Config.EXTRA_ACCOUNTS.append(
            {"refresh_token": result["refresh_token"], "device_id": result["device_id"]}
        )
        Config.save_config()
        n = _run(reload_pool())
    ...
    def _test_accounts(self):
        ...
        n = _run(reload_pool(verbose=True))
        console.print(
    ...
    def download_menu(self):
        ...
        if not raw_url:
            return
        pwd = Prompt.ask(
            f"  [bold white]🔑 {Language.get('input_pwd')}[/]", password=True
        )
        ...
        for url in url_list:
            ...
            data = _run(self.downloader.get_tree_and_prepare(url, pwd))
            if not data:
                console.print("  [bold red]✖ Skipped (failed to load)[/]")
                continue
        ...
        summary.add_row(
            f"[bold cyan]Links OK: {len(all_link_data)}[/]  |  {Language.get('total_files')}: [bold cyan]{grand_total_files}[/]",
            f"{Language.get('total_size')}: [bold cyan]{self.downloader.format_size(grand_total_size)}[/]",
        )
        console.print(
            Panel(
                summary,
                title=Language.get("link_info"),
                border_style="green",
                box=box.ROUNDED,
            )
        )
            ...
            try:
                for p in raw.split(","):
                    if "-" in p:
                        ...
                        idxs.add(int(p))
                master_files = [
                    master_files[i - 1] for i in idxs if 1 <= i <= len(master_files)
                ]
            except:
                master_files = []
        if not master_files:
    ...
    def run_download_with_retry(self, files) -> bool:
        while True:
            ...
            with Live(
                self.downloader.generate_dashboard_table(),
                ...
                async def download_files():
                    ...
                    for i, f in enumerate(files):
                        ...
                        tasks.append(
                            asyncio.create_task(sem_download(f, i, api_for_file))
                        )
                    await asyncio.gather(*tasks, return_exceptions=True)
                    if self.downloader.bg_tasks:
                        await asyncio.gather(
                            *list(self.downloader.bg_tasks), return_exceptions=True
                        )
            ...
            failed_files = []
            for i, f in enumerate(files):
                status = self.downloader.progress_data.get(i + 1, {}).get(
                    "status", "Unknown"
                )
            ...
            if failed_count > 0:
                if Confirm.ask(
                    f"[bold yellow]⚠ Có {failed_count} file bị lỗi. Retry?[/]"
                ):
                    files = failed_files
                    continue
                else:
